...
float3 BRDF(float3 diffuseColor, float3 f0, float3 v, float3 n)
{
...
float4 PSMain(PSInput input) : SV_TARGET
{
	...
	float3 diffuseColor = (1.0 - metallic) * baseColor.xyz;
	float3 f0 = 0.16 * reflectance * reflectance * (1.0 - metallic) + baseColor * metallic;

	color.xyz =  BRDF(diffuseColor, f0, v, input.normal_ws)* intensity;
